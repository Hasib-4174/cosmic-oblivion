...
Color CAlpha(Color c,unsigned char a){c.a=a;return c;}
...
int LoadHS(void){FILE*f=fopen(HIGHSCORE_FILE,"r");if(!f)return 0;int h=0;if(fscanf(f,"%d",&h)!=1)h=0;fclose(f);return h;}
void SaveHS(int s){FILE*f=fopen(HIGHSCORE_FILE,"w");if(!f)return;fprintf(f,"%d\n",s);fclose(f);}
